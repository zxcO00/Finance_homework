...
fig_ma.add_trace(
    go.Candlestick(
        ...
        low=KBar_df['low'], close=KBar_df['close'], name='K 線'
    ), secondary_y=True
)
fig_ma.add_trace(
    go.Scatter(
        x=KBar_df['time'], y=KBar_df['MA_long'],
        mode='lines', name=f'MA {ma_long}'
    ), secondary_y=True
)
fig_ma.add_trace(
    go.Scatter(
        x=KBar_df['time'], y=KBar_df['MA_short'],
...
fig_ma.add_trace(
    go.Bar(
        ...
        name='成交量', marker=dict(color='lightgray')
    ), secondary_y=False
)
...
fig_bb.add_trace(
    go.Scatter(
        x=KBar_df['time'], y=KBar_df['BB_MID'], mode='lines', name='中軌'
    ), secondary_y=True
)
fig_bb.add_trace(
    go.Scatter(
        x=KBar_df['time'], y=KBar_df['BB_UP'], mode='lines', name='上軌'
    ), secondary_y=True
)
fig_bb.add_trace(
    go.Scatter(
        x=KBar_df['time'], y=KBar_df['BB_DOWN'], mode='lines', name='下軌'
    ), secondary_y=True
)
fig_bb.add_trace(
    go.Bar(
...
fig_macd.add_trace(
    go.Scatter(
        x=KBar_df['time'], y=KBar_df['MACD'],
        mode='lines', name='MACD'
    ), row=1, col=1
)
fig_macd.add_trace(
    go.Scatter(
        x=KBar_df['time'], y=KBar_df['MACD_SIGNAL'],
        mode='lines', name='Signal'
    ), row=1, col=1
)
fig_macd.add_trace(
    go.Bar(
        x=KBar_df['time'], y=KBar_df['MACD_HIST'],
        name='Histogram', marker=dict(color='gray')
    ), row=2, col=1
)
